...
import useEscapeKeydown from "@/app/lib/hooks/use-escape-keydown";

const DISMISSABLE_LAYER_NAME = "DismissableLayer";
const CONTEXT_UPDATE = "dismissableLayer.update";
const POINTER_DOWN_OUTSIDE = "dismissableLayer.pointerDownOutisde";
...
type DismissableLayerProps = PrimitiveDivProps & {
  /**
...
const DismissableLayerContext = createContext({
  ...
  layersWithOutsidePointerEventDisabled: new Set<DismissableLayerElement>(),
...
>((props, forwardedRef) => {
  ...
  const [highestLayerWithOutsidePointerEventsDisabled] = [...context.layersWithOutsidePointerEventDisabled].slice(-1); // prettier-ignore
  const highestLayerWithOutsidePointerEventsDisabledIndex = layers.indexOf(highestLayerWithOutsidePointerEventsDisabled); // prettier-ignore
  const index = node ? layers.indexOf(node) : -1;
  const isBodyPointerEventsDisabled =
    context.layersWithOutsidePointerEventDisabled.size > 0;
  ...
  useEffect(() => {
    ...
    if (disableOutsidePointerEvents) {
      if (context.layersWithOutsidePointerEventDisabled.size === 0) {
        originalBodyPointerEvents = ownerDocument.body.style.pointerEvents;
        ownerDocument.body.style.pointerEvents = "none";
      }
      context.layersWithOutsidePointerEventDisabled.add(node);
    }
    ...
    return () => {
      if (
        ...
        context.layersWithOutsidePointerEventDisabled.size === 1
      ) {
  ...
  useEffect(() => {
    return () => {
      ...
      context.layersWithOutsidePointerEventDisabled.delete(node);
      dispatchUpdate();
    };
  }, [node, context]);
